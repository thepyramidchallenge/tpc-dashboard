...
window.TPC_DASHBOARD = {
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Private main 767aa51 carries the synchronized CLI-to-live contract and cleaned planning docs. ROADMAP is one ascending WS0→WS10 registry: all 141 done + 143 open/partial tasks are interleaved by ID with dependencies and S/M/L estimates; deployment-diary narration stays in Git and AGENT_HANDOFF stays concise. Current work is 01c5 method rehearsal, then honest calibration/go-no-go. The accepted live lane still needs versioned prompts, job-scoped pairing auth, combined idempotent slot submission, the repo companion command, backend-first deploy, GenerationRuns migration/readback and non-promoting/real smokes.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  changelog: [
    { date: "2026-07-17", who: "Natalie + Codex (GPT-5)", project: "tpc-online-platform",
      summary: "Consolidated the final all-Codex documentation state through private main 767aa51. QUESTION_FACTORY now accepts official production as live GenerationRuns first, then a founder-run authenticated local Codex/Claude subscription-CLI companion (generator and independent judge) submitting one slot at a time to the TPC backend; no developer model API key, direct Sheets write or completed production batch handoff. ROADMAP was cleaned from 782 to 452 lines without deleting or reclassifying work: all 141 done and 143 open/partial task IDs remain in one ascending WS0→WS10 registry, complete/partial/todo rows are interleaved, dependencies and S/M/L estimates are restored, held commitments keep explicit IDs, and Git owns the retired deployment diary. AGENT_HANDOFF and root/backend/template/architecture/wireflow docs are synchronized. Current 01c5 is correctly a method rehearsal because the known pair is already unblinded; honest calibration follows. Validation: task-ID/order/status audit, diff check and local Markdown links pass." },
    { date: "2026-07-17", who: "Natalie + Codex (GPT-5)", project: "tpc-online-platform",
