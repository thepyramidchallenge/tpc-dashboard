...
window.TPC_DASHBOARD = {
  ...
  focus:
    "WS5.1-18 family incl. 18i vocab ratification now FULLY CLOSED (only 18h 日誌 dashboard stays parked); ratified 日誌 labels are LIVE (Pages f8d8fcf, bundle-verified). Founder now owes: WS5.2-01a decisions B/C (seed-pack pool/reuse + first-run size) and two live smokes (panel-parity on a real phone · whitelist grant/revoke bell). Engineering: WS5.2-02e approval-evaluation panel + backend are source-complete — migrate the QuestionReviewEvaluations tab → deploy → §10.2 smoke, then seed-pack lane → WS5.1-04 sets. Asset-taxonomy ratification lives with WS5.3-01c (founder routing, 18i D6).",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Private main 6059285 carries the restructured AGENT_HANDOFF/ROADMAP (six new decision-log entries; WS5.2-01h/02e + WS6.1-13 decomposed into sub-tasks), the accepted QUESTION_FACTORY contract, docs/VOCABULARY_RATIFICATION.md (founder review underway), and 02e track A committed (4668238) with track B source-complete in-tree; public Pages is gh-pages a562cf9; backend is Cloud Run tpc-api-00059-rwn. Next: 02e migration/deploy/§10.2 smoke → WS5.2-01a → seed-pack lane → WS5.1-04 sets → WS5.1-05 placeholders → WS4.2 fixed-set flow.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      ...
      { title: "WS5.2-02e go-live — migrate → deploy → §10.2 smoke", project: "tpc-online-platform", owner: "natalie", note: "Approval-evaluation panel (track A, committed 4668238; frontend 412 local + demo drive passed) and backend QuestionReviewEvaluations persistence (track B, source-complete in-tree) are done in source. Go-live gate: run the tab migration + read-back → deploy Cloud Run → real ai_generated panel-to-Sheets smoke incl. stale retry + evaluationId in AdminLog detail. Do not flip 02e or the dashboard before that." },
