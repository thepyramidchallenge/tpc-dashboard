...
window.TPC_DASHBOARD = {
  ...
  focus:
    "Pilot readiness is bottlenecked by approved official K2/K3 content and one unproven backend fix, not WS4.2 core. The merge-lane backlog is cleared and the backend cutover already happened: Cloud Run tpc-api-lvpc9b1e47e serves 100% carrying merged #171/#174/#175, so the separately staged tpc-api-lv092f8dd never took traffic. The open gate is that the fixed-set completion UAT which failed on legacy version-zero snapshots has NOT been re-run against the deployed fix — it is shipped but unproven. Alongside that: preserve the live governed learner path, run serialized pilot-scale concurrency evidence, and replace or unpublish the sole dummy mock before public launch. Treasure activation remains on hold.",
  ...
  board: {
    ...
    blocked: [
      { title: "Prove the deployed backend fix", project: "tpc-online-platform", owner: "natalie", note: "THE cutover already happened: Cloud Run tpc-api-lvpc9b1e47e serves 100% from main c9b1e47, carrying merged PRs #171, #174 and #175. The staged tpc-api-lv092f8dd never took traffic. What is NOT done is the proof — the fixed-set completion UAT that failed on legacy version-zero snapshots has not been re-run against this revision, so the fix is deployed but unproven in production. That is now the top open item. PR #168 (release truth) records the SUPERSEDED boundary and is 8 commits behind main; it needs a rebase and a rewrite against Pages 098d243c / source a2c24301 before it can merge, and its 11-file doc update is still uncommitted in a worktree. PR #176 corrects the AGENT_HANDOFF boundary table, which was wrong on both layers; check-docs.mjs boundary still reports 16 stale 'current release' lines across 9 other docs, left to the #168 lane. WS6.3-07 is a two-line uncommitted ROADMAP row in a worktree; rebase and publish separately. Do not merge by ancestry alone: claude/admin-chinese-only-wip is superseded by merged #155/#157, codex/release-integrity-gate by #152/#163, and public deployment branches are release outputs, not merge sources. 73 worktrees exist and 8 are dirty (6 with tracked changes, one of them the primary checkout) — keep them quarantined, do not clean wholesale." },
