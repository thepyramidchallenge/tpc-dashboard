...
window.TPC_DASHBOARD = {
  ...
  focus:
    "WS5 admin source is now live-deployed but not fully smoke-closed. Today source `f08ac8b` implemented the founder-approved full question review flow (draft→review→approved/unapproved, reviewNote, reviewer actions, Assets/Log placeholders, WS5-16 per-admin tab permissions) and `2b30a50` recorded production deploy state: Pages `beb4c8c` / bundle `index-DSMkLxtk.js`, Cloud Run `tpc-api-00029-lnz` with `SUPER_ADMIN_EMAILS=info@pyramidchallenge.org`. The one blocking operational step is the live Sheet migration: create/update Customers `AdminEmails` (`email addedBy addedAt tabs`), append `Users.adminTabs`, and append `Questions.reviewNote` using `cloud-run/scripts/migrate-ws5-16-sheets.js`; until then account grant/tick actions fail on the missing tab and review notes/adminTabs may be dropped by header mapping. After migration, live-smoke WS5-17 end to end, then close WS5-01/02/03 and WS5-16 as appropriate.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Source of truth for full-stack platform work — tracks the Phase-1 roadmap. Current WS5 source/deploy checkpoint: `f08ac8b` full review flow + WS5-16 permissions; `2b30a50` migration script + deploy state; Pages `beb4c8c`; Cloud Run `tpc-api-00029-lnz`; sheet migration still outstanding before full live smoke.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      { title: "WS5 sheet migration + live smoke", project: "tpc-online-platform", owner: "natalie", note: "Run the idempotent WS5-16 migration for AdminEmails.tabs, Users.adminTabs, Questions.reviewNote; then smoke review flow + admin permissions on live." },
