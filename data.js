...
window.TPC_DASHBOARD = {
  ...
  board: {
    now: [
      ...
      { title: "Learner UI design sprint — 答題畫面 first", project: "tpc-online-platform", owner: "max", note: "Local demo now has 14 visual design fixtures and ?demo=1&visual=1 for an image-only practice pool. K2-self-operable single interface (Funexpected bar): audio/demonstration over reading, big targets, parent shell gated, settings streamlined, 429 kid-friendly retry state. Designer Space steps 1–2 are built on feat/answering-ui-revamp (not merged/deployed): 預覽學生介面 toggle + admin-tier 設計 tab with the Playground curation specimen gallery (real QuizRunner under .curation-preview; tokens never touch :root). Next: Atelier curation side-by-side, then one adoption round. Design freeze Aug 21; feeds the WS6.2 review lane." },
