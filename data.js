...
window.TPC_DASHBOARD = {
  ...
  focus:
    "詞彙批准表 vocabulary packet is RULED except D4 (phantom system rows dropped, affinity.pages → content.pages, 帳戶 labels kept; asset taxonomy routed to WS5.3-01c; nothing applied yet). WS5.2-02e is fully committed — both tracks in main — awaiting tab migration → deploy → §10.2 smoke; the WS5.2-01a seed-pack scope doc is being drafted. Founder still owes: the D4 ruling, panel-parity phone smoke, and the whitelist grant/revoke bell check.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "WS5.1-18 family (18f/18g) + WS5.1-19 closed and founder-smoked 2026-07-10; live = Pages a562cf9 · Cloud Run tpc-api-00059-rwn (deployed suites 376 frontend / 110 backend). WS5.2 question-factory policy accepted (QUESTION_FACTORY.md canonical); the 02e approval-evaluation panel + backend persistence are source-complete awaiting tab migration → deploy → §10.2 smoke (both tracks committed). Next: founder rules D4 + WS5.2-01a scope → vocab apply pass → seed-pack lane → WS5.1-04 sets.",
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Private main 03810e9: BOTH WS5.2-02e tracks committed (panel 4668238 + backend/migration 03810e9), the vocabulary packet ruled except D4 (rulings recorded in docs/VOCABULARY_RATIFICATION.md — nothing applied yet), restructured AGENT_HANDOFF/ROADMAP with the 2026-07-10 decision log, and the WS5.2-01a scope doc drafting in-flight. Public Pages gh-pages a562cf9; Cloud Run tpc-api-00059-rwn (pre-02e). Next: 02e migration/deploy/§10.2 smoke → D4 ruling + vocab apply pass → WS5.2-01a → seed-pack lane → WS5.1-04 sets.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      { title: "Founder actions — ratify 詞彙批准表 · panel-parity phone · whitelist bell", project: "tpc-online-platform", owner: "natalie", note: "① Rule the last open vocabulary item D4 (collapsed student.session_submit / notification_read — awaits a fuller explanation); everything else is ruled (phantom system rows dropped · affinity.pages → content.pages · 帳戶 labels kept · asset taxonomy routed to WS5.3-01c) and the apply pass runs after D4. ② Open the admin console on a real phone (100dvh pager; dashboard flip waits on it). ③ Grant/revoke self-approval → target admin's bell rings (tpc-api-00059-rwn). When convenient: editor asset-picker path smoke (WS5.3-03)." },
      { title: "WS5.2-02e go-live — migrate → deploy → §10.2 smoke", project: "tpc-online-platform", owner: "natalie", note: "BOTH tracks are committed to main — panel 4668238 + backend persistence/migration 03810e9. Go-live gate: run the QuestionReviewEvaluations tab migration + read-back → deploy Cloud Run (live is still pre-02e tpc-api-00059-rwn) → real ai_generated panel-to-Sheets smoke incl. stale retry + evaluationId in AdminLog detail. Do not flip 02e or the dashboard before that." },
      { title: "WS5.2 seed pack → WS5.1-04 sets", project: "tpc-online-platform", owner: "natalie", note: "Policy is accepted (QUESTION_FACTORY.md: 55% indicator, three gates, confidence rubric, retained analysis) and the WS5.2-01a scope doc (docs/WS5_2_SEED_PACK_SCOPE.md) is being drafted. After the founder settles counts/distribution + the text-representable real_seed allowlist: offline dry run → gated promotion → generator UI (01h1–01h6) → approve/import → assemble 3-5 K2 + 3-5 K3 non-real_seed QuestionSets." },
  ...
  roadmap: [
    ...
    },
    {
      ...
      items: [
        ...
        { label: "WS5.1-18/19 · 日誌 + Notifications family + self-approval whitelist", state: "done" },
        { label: "WS5.2 · Question factory (seed-pack lane active; recurring AI hold)", state: "active" },
        { label: "WS5.2-02e · AI approval evaluation panel + retained analysis (committed; go-live pending)", state: "active" },
  ...
  changelog: [
    { date: "2026-07-10", who: "Claude (Fable 5)", project: "tpc-online-platform",
      summary: "Stage sync (2026-07-10 night): founder RULED the vocabulary packet except one item — D1 ratified, phantom system/system.migration rows DROPPED, grammar rule official, all six notification types agreed, asset taxonomy routed to WS5.3-01c (stays open), affinity.pages → content.pages while unused, Appendix-A labels approved with 允許/取消自我批准 wording, and 帳戶 kept over the 用戶 proposal; only D4 (collapsed session_submit/notification_read actions) awaits a fuller explanation — nothing applied yet, the apply checklist is staged. WS5.2-02e track B (backend QuestionReviewEvaluations enforcement/persistence + tab migration) is now COMMITTED (03810e9, landed by a Claude session to unblock deploy), so both 02e tracks are in private main; live Cloud Run remains pre-02e tpc-api-00059-rwn until the migration → deploy → §10.2 smoke gate runs. A WS5.2-01a seed-pack scope doc (docs/WS5_2_SEED_PACK_SCOPE.md) is drafting in-flight. Dashboard roadmap gains stage rows: WS5.1-18/19 done · WS5.2-02e active." },
    { date: "2026-07-10", who: "Claude (Fable 5)", project: "tpc-online-platform",
