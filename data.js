...
window.TPC_DASHBOARD = {
  ...
  focus:
    "Pilot readiness is now bottlenecked by approved official K2/K3 content and the unfinished backend cutover, not WS4.2 core. The merge-lane backlog is cleared (#134 merged, #135 closed, #136/#155/#157/#171/#173 merged); what remains is to smoke tpc-api-lv092f8dd, cut traffic over only if it passes, and repeat the fixed-set completion UAT that failed on legacy version-zero snapshots. Alongside that: preserve the live governed learner path, run serialized pilot-scale concurrency evidence, and replace or unpublish the sole dummy mock before public launch. Treasure activation remains on hold.",
  ...
  board: {
    ...
    blocked: [
      { title: "Backend cutover + open merge lanes", project: "tpc-online-platform", owner: "natalie", note: "The #134/#135 routing question is closed: #134 merged, #135 closed and replaced by #149, #136 merged and live. Current sequence: smoke tpc-api-lv092f8dd (PR #171's legacy version-zero fix, Ready at 0%), cut traffic over from tpc-api-pcmp5ccf only if the smoke passes, then repeat the fixed-set completion UAT that failed on legacy version-zero snapshots. PR #168 (release truth) is a draft and merges last, after the cutover and its 11-file doc update are rechecked against the new Pages release. PR #174 retires the duplicate legacy exposure count — a production schema mutation in the same exposure area, so sequence it deliberately around the cutover rather than in parallel. WS6.3-07 is a two-line uncommitted ROADMAP row in a worktree, 4 commits behind main; rebase and publish it separately. Do not merge by ancestry alone: claude/admin-chinese-only-wip is superseded by merged #155/#157, codex/release-integrity-gate by #152/#163, and public deployment branches are release outputs, not merge sources. 73 worktrees exist and 8 are dirty (6 with tracked changes, one of them the primary checkout) — keep them quarantined, do not clean wholesale." },
