...
window.TPC_DASHBOARD = {
  ...
  board: {
    now: [
      ...
      { title: "Learner UI design sprint — 答題畫面 first", project: "tpc-online-platform", owner: "max", note: "Local demo now has 14 visual design fixtures and ?demo=1&visual=1 for an image-only practice pool. K2-self-operable single interface (Funexpected bar): audio/demonstration over reading, big targets, parent shell gated, settings streamlined, 429 kid-friendly retry state. Designer Space steps 1–3 are built on feat/answering-ui-revamp (not merged/deployed): 預覽學生介面 toggle + admin-tier 設計 tab + the Playground/Atelier pair rendered side by side on one question (real QuizRunner under .curation-preview; tokens never touch :root). The loop has already produced two findings the test suite could not: identical 答對/答錯 tiles on the rebuilt stage (fixed, WS3.1-04c) and an over-emphasised 2.5D depth model corrected to flat fill + paper grain after re-reading the reference. Next: one adoption round — the only step that validates the whole loop. Design freeze Aug 21; feeds the WS6.2 review lane." },
