...
window.TPC_DASHBOARD = {
  ...
  focus:
    "All five Codex full-review P1s are now LIVE — the last one (Drive file-ID guard, WS5.3-07) deployed 2026-07-17 as tpc-api-00075-h8f (= live lineage 368e6d0 + the asset trust boundary), with writes already unfrozen. Remaining before resuming the WS5.2 generator rollout: reconcile the release lineage + ws53 branches into the canonical line, deploy the upsertUser allowlist, and resolve XFF + classroom-NAT evidence for the token limiter. Consolidated follow-up list: top of tpc-online-platform/AGENT_HANDOFF.md.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Live stack: Pages 4a21abf + Cloud Run tpc-api-00075-h8f (100%, max instances 1) = the atomic-decisions lineage 368e6d0 + the WS5.3-07 Drive file-ID guard (deployed 2026-07-17 after pre-deploy asset-location validation + tag-URL and production smokes); writes are unfrozen. The public generator preview was withdrawn; production AI entry remains gated. Next: reconcile the release lineage + ws53-asset-fileid-guard/ws53-07-release into the canonical line, deploy the upsertUser allowlist, resolve XFF + classroom-NAT evidence for the token limiter, and only then resume provider/async/staging rollout.",
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Live backend source is recovered as 368e6d0 on codex/approval-atomic-release (draft PR #1), not yet reconciled into main; do not deploy main blind. The shared checkout also contains the undeployed upsertUser and token-limiter security work. After branch/security closure: choose independent generator + admission providers, replace the 51-write browser max with a worker or verified 429 continuation, migrate/read back staging tabs, deploy backend-first and smoke.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      { title: "Production read-only — lift decision + backend reconciliation", project: "tpc-online-platform", owner: "natalie", note: "tpc-api-00078-jid serves 100% traffic with FREEZE_WRITES=true; Pages 4a21abf is live. The approval migration/read-back passed and release source is 368e6d0 / draft PR #1. Natalie must explicitly decide the lift after review. Engineering must reconcile that live backend line into the canonical branch before any later deploy." },
      { title: "Full-review closure — security rollout tail", project: "tpc-online-platform", owner: "natalie", note: "ALL five P1s are live as of 2026-07-17: immutable sessions, explicit conflict-safe drafts, content fingerprints, atomic review decisions, and the service-account Drive file-ID guard (WS5.3-07, tpc-api-00075-h8f — kind/MIME from real Drive metadata, incoming/library location checks on admin preview and public /asset even for registry rows, foreign-fileId registration refused, constrained parent moves; pre-deploy validation passed on all 3 registered assets, SA Drive reach audited → blast radius verified minimal). Remaining tail: merge ws53-asset-fileid-guard + ws53-07-release into the canonical line, deploy the P2 upsertUser allowlist (committed 2d39d2f, not live), and the token limiter (source-complete/200 green but uncommitted, blocked on XFF + shared-NAT evidence)." },
      { title: "Natalie's manual evidence — after deliberate unfreeze", project: "tpc-online-platform", owner: "natalie", note: "Run: two-tab Save-for-later conflict/retry; WS5.2 §10.2 approve/unapprove decision-ID evidence; whitelist grant/revoke bell; asset-picker save/reopen. Separately smoke panel parity on a real phone (100dvh versus URL bar). Do not lift the write freeze merely to run these without first reviewing the release." },
      { title: "WS5.2 question factory — source built; production gated", project: "tpc-online-platform", owner: "natalie", note: "01d/01h/01i staging/promotion and the four-click flow are source-built. The public `?generatorPreview=1` was withdrawn after exposing demo-admin surfaces; fixtures and query regression guards were hardened. Real lane remains OFF: no providers, no GenerationJobs/Slots migration, no generated row. Open: Run 001 human evidence, independent model choice, 51-write async/429 boundary, backend-first live smoke. NEW today: Natalie chose a zero-API local CLI generation lane (Claude Code / Codex CLI under existing subscriptions → 01c validation → trusted promotion) — decision is chat-only, still to record in QUESTION_FACTORY/ROADMAP and build the scaffold + skill (designing session lost macOS Documents access). Reviewer UX: approval-evaluation redesign is LIVE (analyses on arrival, similarity matrices, confidence bars); the sticky evaluation-rail follow-up 0c43d5e (459 tests) is committed on origin/main, awaiting Natalie's deploy consent." },
    ...
    blocked: [
      { title: "Token-limiter rollout — trusted XFF + shared NAT", project: "tpc-online-platform", owner: "natalie", note: "Issue 7 is source-complete with 11 regressions and backend 200/200, but direct run.app does not document the trusted XFF position. Measure it and set RATE_LIMIT_XFF_CLIENT_FROM_RIGHT (or add a rewriting HTTPS load balancer), then pass a classroom shared-NAT burst test and verify rate-limited getUser never misroutes an existing user to setup." },
