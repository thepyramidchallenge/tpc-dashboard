...
window.TPC_DASHBOARD = {
  ...
  board: {
    now: [
      ...
      { title: "Learner UI design sprint — 答題畫面 first", project: "tpc-online-platform", owner: "max", note: "Local demo now has 14 visual design fixtures and ?demo=1&visual=1 for an image-only practice pool. K2-self-operable single interface (Funexpected bar): audio/demonstration over reading, big targets, parent shell gated, settings streamlined, 429 kid-friendly retry state. The Designer Space plan (docs/DESIGNER_SPACE.md — a proposal gallery of design recipes rendered through the real components, explicitly not a theme engine) is now started: step 1, the 預覽學生介面 toggle, is built on the branch; steps 2–4 are the tab shell, a second curation and one real adoption round. Design freeze Aug 21; feeds the WS6.2 review lane." },
