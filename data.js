...
window.TPC_DASHBOARD = {
  ...
  focus:
    "Do not deploy the current backend until the four remaining full-review P1s are resolved. The WS5.2 generator now has a live read-only Pages preview (deterministic fixtures; zero model/API/Sheets writes), while the real production button stays hidden. Next: resolve P1s, choose independent generator/admission adapters, add backend-owned async/429-safe batching, migrate the staging tabs, then deploy backend-first and run the real smoke.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Live frontend is Pages fec2788 (source 4faefe5; index-BW725_Eu.js / AdminApp-BP6sDQdp.js) with a read-only `?generatorPreview=1` walkthrough; production accounts still have the AI button hidden. Cloud Run remains tpc-api-00060-wxv and no staging tab/provider/backend was deployed. Validation: frontend 444 / backend 173 + production build; live browser smoke covered batch, both retries, draft scoring and approval handoff. Resolve four P1s first; then provider/admission adapters, async/429-safe batch, staging migration and backend-first real smoke. 02e §10.2 and 01c human evidence remain open.",
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Main 4faefe5 includes the source-built 01d/01h/01i slice and a safe public preview; Pages fec2788 is live and browser-smoked, but only against deterministic local fixtures. Cloud Run remains tpc-api-00060-wxv; no GenerationJobs/Slots migration, provider configuration or real generation deploy occurred. Next: resolve four P1s → select/wire independent generator + admission providers → replace the 51-write browser max with a worker or explicit 429 continuation → migrate/read back → backend-first deploy and real smoke.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      ...
      { title: "WS5.2 question factory — safe preview live; real lane gated", project: "tpc-online-platform", owner: "natalie", note: "01d/01h/01i source is built and the deterministic no-write Pages preview is live at `?generatorPreview=1` (fec2788). Browser smoke passed 4-output Batch, import retry, gate retry, AI draft scores and sequential approval entry; frontend 444 / backend 173 + build. Real production remains OFF: AI entry hidden, Cloud Run unchanged, no provider/admission adapter, no GenerationJobs/Slots migration, and no generated Question row. The 51-write max vs 30/min keeps 01h4 partial. 01c Run 001 still needs human verification/confidence/time evidence." },
