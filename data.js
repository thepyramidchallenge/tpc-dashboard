...
window.TPC_DASHBOARD = {
  ...
  focus:
    "WS5.2-02e approval-evaluation rails are LIVE end-to-end (QuestionReviewEvaluations tab migrated · Cloud Run tpc-api-00060-wxv · Pages f8d8fcf · §10.2 smoke row k3_m000001_ai seeded) — the one gate left is the founder's §10.2 in-browser smoke, then 02e closes. WS5.2-02f AI evaluator (factory decision 14: AI drafts the evaluation, human decides) is source-complete (bf42874) awaiting founder deploy consent + ANTHROPIC_API_KEY. Founder also owes: WS5.2-01a decisions B/C, panel-parity phone smoke, whitelist bell smoke. WS5.1-18 family incl. 18i vocab ratification stays fully closed (only 18h parked); asset-taxonomy ratification lives with WS5.3-01c.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Live = Pages f8d8fcf · Cloud Run tpc-api-00060-wxv (suites 424 frontend / 132 backend at close). WS5.2 question-factory policy accepted (QUESTION_FACTORY.md canonical) and the 02e approval-evaluation rails are LIVE end-to-end (tab migrated, panel + enforcement deployed, smoke row seeded) — founder §10.2 smoke is the closure gate. 02f AI evaluator source-complete awaiting deploy consent + ANTHROPIC_API_KEY. Next: §10.2 smoke → WS5.2-01a → seed-pack lane → WS5.1-04 sets.",
    },
    {
      ...
      next:  "Source of truth for full-stack platform work. Private main carries the accepted QUESTION_FACTORY contract with the §10 track table (A `4668238` · B `03810e9` · C `bf42874`), the AI-evaluator decision (factory register entry 14) + §10.1a contract, and the WS5.2-01c calibration work in flight (Codex). Public Pages gh-pages f8d8fcf; backend Cloud Run tpc-api-00060-wxv (carries 02e enforcement). Next: founder §10.2 smoke + 02f deploy consent/API key → WS5.2-01a → seed-pack lane → WS5.1-04 sets → WS5.1-05 placeholders → WS4.2 fixed-set flow.",
    },
    {
      id:    "entrance-qr-scan",
  ...
  board: {
    now: [
      { title: "Founder actions — panel-parity phone · whitelist bell", project: "tpc-online-platform", owner: "natalie", note: "① Open the admin console on a real phone (100dvh pager behaviour; dashboard flip waits on it). ② Grant/revoke self-approval → target admin's bell rings (tpc-api-00059-rwn). Also still owed when convenient: editor asset-picker path smoke (WS5.3-03). (18i vocabulary ratification: DONE 2026-07-10 — rulings in the platform ROADMAP decision log.)" },
      { title: "WS5.2-02e §10.2 smoke + 02f deploy consent", project: "tpc-online-platform", owner: "natalie", note: "02e rails are LIVE (tab migrated read-back OK → Cloud Run tpc-api-00060-wxv → Pages, carried by f8d8fcf) and the smoke row k3_m000001_ai is seeded. Founder: run the §10.2 panel→Sheets smoke (approve → QuestionReviewEvaluations row + evaluationId in AdminLog + bell on info@hkycaa.org; optional stale retry), then disable the row and flip 02e. 02f AI evaluator (factory decision 14 — AI drafts, human decides) is committed bf42874: deploy backend-first per the AGENT_HANDOFF runbook + set ANTHROPIC_API_KEY; after that the smoke can start with ✦ AI 初評 instead of hand-filling." },
      { title: "WS5.2 seed pack → WS5.1-04 sets", project: "tpc-online-platform", owner: "natalie", note: "Policy is accepted (QUESTION_FACTORY.md: 55% indicator, three gates, confidence rubric, retained analysis). Blocked only on the founder's WS5.2-01a scope decision (counts/distribution + text-representable real_seed allowlist), then: offline dry run → gated promotion → generator UI (01h1–01h6) → approve/import → assemble 3-5 K2 + 3-5 K3 non-real_seed QuestionSets." },
