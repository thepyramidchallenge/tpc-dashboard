...
window.TPC_DASHBOARD = {
  ...
  focus:
    "Production is intentionally read-only on tpc-api-00078-jid. First decide whether to lift FREEZE_WRITES and reconcile the auditable live backend commit 368e6d0 into the canonical line; then integrate the now-built Drive file-ID guard (ws53-asset-fileid-guard, tested, uncommitted — the last P1) and the two security P2 rollout gates before resuming the real WS5.2 generator rollout. Consolidated follow-up list: top of tpc-online-platform/AGENT_HANDOFF.md.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Live stack: Pages 4a21abf + Cloud Run tpc-api-00078-jid (100%, max instances 1), with atomic review-decision IDs and the P1 saved-draft/fingerprint/session fixes; FREEZE_WRITES=true. Review draft PR #1 / source 368e6d0 and decide the lift. The public generator preview was withdrawn; production AI entry remains gated. Then integrate the built Drive file-ID guard (ws53-asset-fileid-guard, tested/uncommitted), deploy the upsertUser allowlist, resolve XFF + classroom-NAT evidence for the token limiter, and only then resume provider/async/staging rollout.",
    },
    {
  ...
  board: {
    now: [
      ...
      { title: "Full-review closure — 1 P1 + security rollout", project: "tpc-online-platform", owner: "natalie", note: "P1 #1–#4 are live: immutable sessions, explicit conflict-safe drafts, content fingerprints, and atomic review decisions. Last P1 (service-account Drive reads) is now IMPLEMENTED + TESTED on branch ws53-asset-fileid-guard in an isolated worktree (kind/MIME from real Drive metadata, incoming/library location checks on admin preview and public /asset, foreign-fileId registration refused, constrained parent moves; backend 154 green there) — uncommitted/undeployed, and the authoring chatroom is idle on Natalie's integration answer. P2 upsertUser allowlist is committed (2d39d2f) but not live; token limiter is source-complete/200 green but uncommitted and blocked on XFF + shared-NAT evidence." },
