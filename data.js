...
window.TPC_DASHBOARD = {
  ...
  focus:
    "Production is intentionally read-only on tpc-api-00078-jid. First decide whether to lift FREEZE_WRITES and reconcile the auditable live backend commit 368e6d0 into the canonical line; then close the last P1 (arbitrary Drive file-ID reads) and the two security P2 rollout gates before resuming the real WS5.2 generator rollout.",
  ...
  projects: [
    ...
    },
    {
      ...
      next:  "Live stack: Pages 4a21abf + Cloud Run tpc-api-00078-jid (100%, max instances 1), with atomic review-decision IDs and the P1 saved-draft/fingerprint/session fixes; FREEZE_WRITES=true. Review draft PR #1 / source 368e6d0 and decide the lift. The public generator preview was withdrawn; production AI entry remains gated. Then close Drive file-ID reads, deploy the upsertUser allowlist, resolve XFF + classroom-NAT evidence for the token limiter, and only then resume provider/async/staging rollout.",
    },
    {
  ...
  board: {
    now: [
      ...
      { title: "Full-review closure — 1 P1 + security rollout", project: "tpc-online-platform", owner: "natalie", note: "P1 #1–#4 are live: immutable sessions, explicit conflict-safe drafts, content fingerprints, and atomic review decisions. Remaining P1: restrict service-account Drive reads to registered/approved assets. P2 upsertUser allowlist is committed (2d39d2f) but not live; token limiter is source-complete/200 green but uncommitted and blocked on XFF + shared-NAT evidence." },
      ...
      { title: "WS5.2 question factory — source built; production gated", project: "tpc-online-platform", owner: "natalie", note: "01d/01h/01i staging/promotion and the four-click flow are source-built. The public `?generatorPreview=1` was withdrawn after exposing demo-admin surfaces; fixtures and query regression guards were hardened. Real lane remains OFF: no providers, no GenerationJobs/Slots migration, no generated row. Open: Run 001 human evidence, independent model choice, 51-write async/429 boundary, backend-first live smoke." },
